...
custom_imports = dict(
    imports=['mmocr.datasets.icdar_e2e_dataset'], allow_failed_imports=False)
model = dict(
    type='LRANet',
    backbone=dict(
        type='ResNet',
...
train_pipeline = [
    ...
    dict(
        ...
        to_rgb=True),
    dict(type='RandomScaling', size=960, scale=(0.375, 2.5)),
    dict(
        type='RandomCropFlip', crop_ratio=0.2, iter_num=1,
        min_area_ratio=0.01),
    dict(
...
data = dict(
    ...
    train=dict(
        ...
        dataset=dict(
            ...
            ann_file=['data/totaltext/totaltext_train.json'],
            img_prefix=['data/totaltext/imgs/training'],
            pipeline=[
                dict(type='LoadImageFromFile'),
                dict(
                    type='LoadTextAnnotations',
                ...
                dict(
                    ...
                    keys=[
                        ...
                        'polygons_area', 'gt_texts', 'lra_polys'
                    ])
            ])),
    val=dict(
        type='IcdarE2EDataset',
...
resume_from = None
workflow = [('train', 1)]
work_dir = 'work_dirs/totaltext_det'
gpu_ids = range(0, 1)
